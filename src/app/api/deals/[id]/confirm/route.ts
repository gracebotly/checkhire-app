...
import { sendDeliveryConfirmedEmail } from "@/lib/email/escrow-notifications";
...
export const POST = withApiHandler(
  async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
    ...
    const { data: freelancerProfile } = await supabase.from("user_profiles").select("stripe_connected_account_id, stripe_onboarding_complete, email").eq("id", deal.freelancer_user_id!).maybeSingle();
    ...
    // Increment completed_deals_count for both parties on full deal completion
    ...
    if (isFullyComplete?.status === "completed") {
      const { data: clientData } = await serviceClient.from("user_profiles").select("completed_deals_count").eq("id", deal.client_user_id).maybeSingle();
      const { data: freelancerData } = await serviceClient.from("user_profiles").select("completed_deals_count").eq("id", deal.freelancer_user_id!).maybeSingle();
      if (clientData) {
        await serviceClient.from("user_profiles").update({ completed_deals_count: (clientData.completed_deals_count || 0) + 1 }).eq("id", deal.client_user_id);
      }
      if (freelancerData) {
        await serviceClient.from("user_profiles").update({ completed_deals_count: (freelancerData.completed_deals_count || 0) + 1 }).eq("id", deal.freelancer_user_id!);
      }
    }

    // Email freelancer
    if (freelancerProfile.email) {
      await sendDeliveryConfirmedEmail({ to: freelancerProfile.email, dealTitle: deal.title, dealSlug: deal.deal_link_slug, amount: releaseAmount });
      await serviceClient.from("email_notifications").insert({ user_id: deal.freelancer_user_id!, deal_id: id, notification_type: "deal_completed", email_address: freelancerProfile.email, sent_at: new Date().toISOString() });
    }

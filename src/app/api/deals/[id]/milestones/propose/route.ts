...
export const POST = withApiHandler(
  async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
    ...
    const otherUserId = deal.client_user_id === user.id ? deal.freelancer_user_id : deal.client_user_id;
    if (otherUserId) {
      const serviceClient = createServiceClient();
      const { data: otherProfile } = await serviceClient.from("user_profiles").select("email").eq("id", otherUserId).maybeSingle();
